...
let parseLine (line: string) =
    ...
    let start = parts[0].Split(',') |> Array.map decimal |> Vector3.create
    let dir = parts[1].Split(',') |> Array.map decimal |> Vector3.create
    { Start = start; Direction = dir }

let part1 (rays: Ray3 list) =
    let p = 200000000000000m
    let w = 200000000000000m
    let bounds = Rectangle2.create p p w w
...
let part2 (rays: Ray3 list) =
    let xyRow (i, j) =
        ...
        [ r1.DY - r0.DY; r0.DX - r1.DX; r0.Y - r1.Y; r1.X - r0.X ]
        |> List.map (int64 >> rational)

    let xyB (i, j) =
        let r0 = rays[i]
        let r1 = rays[j]
        r0.DX * r0.Y + r1.DY * r1.X - r0.DY * r0.X - r1.DX * r1.Y |> int64 |> rational
    ...
    let xzB (i, j) =
        let r0 = rays[i]
        let r1 = rays[j]
        r0.DX * r0.Z + r1.DZ * r1.X - r0.DZ * r0.X - r1.DX * r1.Z |> int64 |> rational
